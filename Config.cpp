...
RID Config::GetRID(const char * field, RID default, int index) const
{
	JSON_Value* value = FindValue(field, index);

	if (value && json_value_get_type(value) == JSONNumber)
		return (RID)json_value_get_number(value);
...
bool Config::AddUID(const char * field, RID value)
{
	return json_object_set_number(root, field, (double)value) == JSONSuccess;
}
...
bool Config::AddArrayRID(const char * field, const RID * values, int size)
{
	if (values != nullptr && size > 0)
	{
		JSON_Value* va = json_value_init_array();
