...
#pragma comment( lib, "lib/PhysFS/libx86/physfs.lib" )
...
bool ModuleFileSystem::AddPath(const char* path_or_zip)
{
	...
	if (PHYSFS_mount(path_or_zip, nullptr, 1) == 0) {
		LOGC("File System error while adding a path or zip: %s\n", PHYSFS_getLastError());
	}
	else
		ret = true;
...
void ModuleFileSystem::DiscoverFiles(const char* directory, vector<string> & file_list, vector<string> & dir_list) const
{
	...
	for (i = rc; *i != nullptr; i++)
	{
		...
		if (PHYSFS_isDirectory(str.c_str()))
...
uint ModuleFileSystem::Save(const char* file, const void* buffer, unsigned int size, bool append) const
{
	...
	if (fs_file != nullptr)
	{
		...
		if (written != size) {
			LOGC("File System error while writing to file %s: %s", file, PHYSFS_getLastError());
		}
		else
		{
			if (append == true) {
				LOGC("Added %u data to [%s%s]", size, PHYSFS_getWriteDir(), file);
				//else if(overwrite == true)
					//LOG("File [%s%s] overwritten with %u bytes", PHYSFS_getWriteDir(), file, size);
			}
			else if (overwrite == false) {
				LOGC("New file created [%s%s] of %u bytes", PHYSFS_getWriteDir(), file, size);
			}
		...
		if (PHYSFS_close(fs_file) == 0) {
			LOGC("File System error while closing file %s: %s", file, PHYSFS_getLastError());
		}
	}
	else {
		LOGC("File System error while opening file %s: %s", file, PHYSFS_getLastError());
	}
...
bool ModuleFileSystem::SaveUnique(string& name, const void * buffer, uint size, const char * path, const char * prefix, const char * extension)
{
	...
	sprintf_s(result, 250, "%s%s_%llu.%s", path, prefix, App->RandomNumberGenerator.GetIntRNInRange(), extension);
...
FolderContainer ModuleFileSystem::RecursiveGetFoldersFiles(const char* directory, std::vector<std::string>* filter_ext, std::vector<std::string>* ignore_ext)
{
	...
	if (node.localPath == "") {
		...
		local[1] = toupper(local[1]);
		node.localPath = local.c_str();
	}
